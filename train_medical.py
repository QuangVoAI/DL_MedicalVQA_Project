...
def train(args):
    ...
    if config['data'].get(os.environ.get("HF_TOKEN", "")):
        print(f"🌐 Đang tải dữ liệu từ Hub: {config['data'][os.environ.get("HF_TOKEN", "")]}")
        ...
        print(f"📁 Đang tải dữ liệu cục bộ từ: {config['data']['vqa_json']}")
    ...
    if args.variant in ['A1', 'A2']:
        ...
        print(f"📉 Differential LR: PhoBERT ({config['train'].get('phobert_lr', 1e-5)}) | Other ({config['train']['learning_rate']})")
        ...
        print("🧬 Đang chuẩn bị cho huấn luyện DPO...")
        wrapper = MultimodalVQA(model_id=config['model_b']['model_name'])
        model, processor = wrapper.load_model()
        ...
        if os.path.exists(b2_ckpt):
            print(f"🔄 Đang tải checkpoint B2 làm base cho DPO: {b2_ckpt}")
            model.load_state_dict(torch.load(b2_ckpt, map_location=device))
        else:
            print("⚠️ Cảnh báo: Không tìm thấy checkpoint B2. DPO sẽ chạy trên model gốc (không khuyến nghị).")
        ...
        print(f"✅ Đã lưu checkpoint DPO: checkpoints/medical_vqa_dpo.pth")
        return # Thoát sau khi DPO xong
    
    # 4. Sử dụng Trainer tương ứng cho từng Variant
    if args.variant == 'B1':
        # B1: Zero-shot Inference (Không huấn luyện)
        print("🤖 Đang chạy Zero-shot Inference cho B1 (LLaVA-Med)...")
        ...
        print(f"📊 Kết quả Zero-shot (B1): {metrics}")
        return

    elif args.variant == 'B2':
        # B2: Fine-tune sử dụng SFTTrainer (trl)
        print("🚀 Đang khởi tạo SFTTrainer cho B2 (Fine-tuning LLaVA-Med)...")
        ...
        trainer.train()
        print("✅ Đã hoàn thành Fine-tuning B2.")
        return
    ...
    print(f"🚀 Bắt đầu huấn luyện cấu hình {args.variant}...")
    best_val_acc = 0
    ...
    for epoch in range(config.get('epochs', 10)):
        ...
        if val_acc > best_val_acc:
            ...
            print(f"🌟 Epoch {epoch+1}: Đã lưu model tốt nhất mới (Acc: {val_acc:.4f})")
        else:
            counter += 1
            if counter >= patience:
                print(f"🛑 Early stopping tại epoch {epoch+1} do Accuracy không cải thiện.")
                break

    # 6. Lưu Checkpoint cuối cùng
    os.makedirs("checkpoints", exist_ok=True)
    torch.save(model.state_dict(), f"checkpoints/medical_vqa_{args.variant}_final.pth")
    print(f"✅ Đã hoàn thành huấn luyện {args.variant}.")
